...
Após o checkpoint dos ajustes de autoplay e área segura, a primeira navegação ao domínio oficial exibiu uma tela inicial vazia transitória e, em seguida, carregou a landing com o vídeo funcional. A marca e os conteúdos no topo permaneceram visíveis em desktop. A mensagem de fallback de autoplay ainda não estava presente nessa primeira verificação publicada, sinalizando propagação incompleta da nova versão de cliente; será verificada novamente antes da conclusão.

Uma nova navegação com parâmetro de versão confirmou que o domínio oficial ainda serve o bundle `index-ClrjO3sV.js`, anterior aos ajustes recentes: o áudio permanece com `autoplay: false` e `preload: none`, e não estão presentes o link de salto, a área segura do cabeçalho ou a mensagem de fallback. A correção está salva no checkpoint `b515feec`, validada localmente, mas a publicação desse checkpoint ainda precisa ser acionada pela interface de gerenciamento para que o domínio público receba a nova versão.

Após a publicação, a landing oficial passou a servir a versão atualizada. A inspeção confirmou `audio.autoplay: true`, `preload: auto`, fallback de autoplay presente e cabeçalho com área segura presente. Neste navegador, o áudio permaneceu pausado sem erro porque o browser bloqueou som automático sem gesto do visitante; o botão “Ouvir” continua disponível para iniciar a mesma narração imediatamente. O vídeo chegou a `readyState: 4` sem erro, confirmando a reprodução visual no endereço oficial.

Depois da publicação do checkpoint `bc99ba4b`, o domínio oficial carregou a landing atualizada com vídeo e controles de narração disponíveis. A validação de interação da posição de rolagem será executada na sequência para confirmar que o botão do trailer não desloca mais a tela em navegação mobile.

A validação de interação no domínio oficial confirmou que o botão “Ouvir trailer de 30s” preserva a posição de rolagem: com a página fixada em 360 px, a posição permaneceu em 360 px após o acionamento (`preserved: true`). Uma medição preliminar foi desconsiderada porque a rolagem suave global ainda estava em andamento; o segundo teste desativou-a momentaneamente para isolar o efeito do botão.
