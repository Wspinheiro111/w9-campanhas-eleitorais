...
export const financeLegalRouter = router({
  ...
  entries: router({
    ...
    create: protectedProcedure.input(campaignIdInput.extend({ entryType: z.enum(["income", "expense"]), category: z.string().min(2).max(120), counterpartyName: z.string().min(2).max(220), counterpartyDocument: z.string().max(24).optional(), amountCents: z.number().int().positive().max(2_000_000_000), paymentMethod: z.string().max(80).optional(), receiptNumber: z.string().max(100).optional(), documentNumber: z.string().max(100).optional(), dueDate: z.date().optional(), paidAt: z.date().optional(), notes: z.string().max(3000).optional() })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); return { id: await db.createFinancialEntry({ ...input, createdByUserId: ctx.user.id, counterpartyDocument: input.counterpartyDocument ?? null, paymentMethod: input.paymentMethod ?? null, receiptNumber: input.receiptNumber ?? null, documentNumber: input.documentNumber ?? null, dueDate: input.dueDate ?? null, paidAt: input.paidAt ?? null, notes: input.notes ?? null }) }; }),
    review: protectedProcedure.input(z.object({ entryId: z.number().int().positive(), status: z.enum(["under_review", "approved", "rejected", "paid", "cancelled"]), reviewNotes: z.string().max(3000).optional() })).mutation(async ({ ctx, input }) => { const entry = await db.getFinancialEntry(input.entryId); if (!entry) throw new TRPCError({ code: "NOT_FOUND" }); const access = await requireAccess(ctx.user.id, entry.campaignId); requireCapability(access, "manage"); await db.updateFinancialEntryReview({ ...input, id: input.entryId, reviewedByUserId: ctx.user.id, reviewNotes: input.reviewNotes ?? null }); return { success: true }; }),
  }),
  documents: router({
  ...
  playbooks: router({
    ...
    materials: router({
      ...
      upload: protectedProcedure.input(z.object({ playbookId: z.number().int().positive(), fileName: z.string().min(1).max(255), materialType: z.string().min(2).max(80).default("Guia"), topic: z.string().max(160).optional(), base64: z.string().min(8).max(7_000_000) })).mutation(async ({ ctx, input }) => {
        const playbook = await db.getFieldPlaybook(input.playbookId); if (!playbook) throw new TRPCError({ code: "NOT_FOUND" }); const access = await requireAccess(ctx.user.id, playbook.campaignId); requireCapability(access, "manage");
