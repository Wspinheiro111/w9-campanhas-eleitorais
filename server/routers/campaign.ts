...
export const fieldRouter = router({
  ...
  playbooks: router({
    ...
    materials: router({
      ...
      upload: protectedProcedure.input(z.object({ playbookId: z.number().int().positive(), fileName: z.string().min(1).max(255), base64: z.string().min(8).max(7_000_000) })).mutation(async ({ ctx, input }) => {
        ...
        const id = await db.createFieldPlaybookMaterial({ campaignId: playbook.campaignId, playbookId: playbook.id, playbookVersion: playbook.version, fileName: input.fileName, storageKey: stored.key, url: stored.url, sizeBytes: bytes.length, createdByUserId: ctx.user.id });
