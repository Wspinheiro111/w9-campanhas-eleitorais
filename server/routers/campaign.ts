...
export const operationsRouter = router({
  ...
  shifts: router({
    ...
    create: protectedProcedure.input(campaignIdInput.extend({ title: z.string().min(3).max(220), territory: z.string().max(180).optional(), responsibility: z.string().max(220).optional(), startsAt: z.date(), endsAt: z.date(), notes: z.string().max(3000).optional(), memberIds: z.array(z.number().int().positive()).max(100).default([]) })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "team"); if (input.endsAt <= input.startsAt) throw new TRPCError({ code: "BAD_REQUEST", message: "O término deve ser posterior ao início." }); const members = await db.listMembers(input.campaignId); const allowed = new Set(members.map(item => item.id)); if (input.memberIds.some(id => !allowed.has(id))) throw new TRPCError({ code: "BAD_REQUEST", message: "A escala contém membro de outra campanha." }); const conflicts = await findTeamShiftConflicts(input.campaignId, input.startsAt, input.endsAt, input.memberIds); if (conflicts.length) throw new TRPCError({ code: "CONFLICT", message: `Conflito de horário com ${conflicts.map(item => item.memberName).join(", ")}. Remaneje ou altere a equipe antes de publicar.` }); return { id: await db.createTeamShift({ ...input, territory: input.territory ?? null, responsibility: input.responsibility ?? null, notes: input.notes ?? null, createdByUserId: ctx.user.id }) }; }),
    copy: protectedProcedure.input(campaignIdInput.extend({ sourceShiftId: z.number().int().positive(), targetStartsAt: z.array(z.date()).min(1).max(7) })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); const source = (await db.listTeamShifts(input.campaignId)).find(item => item.id === input.sourceShiftId); if (!source) throw new TRPCError({ code: "NOT_FOUND", message: "Escala de origem não encontrada." }); const duration = source.endsAt.getTime() - source.startsAt.getTime(); const memberIds = source.assignments.map(item => item.memberId); const copies = input.targetStartsAt.map(startsAt => ({ startsAt, endsAt: new Date(startsAt.getTime() + duration) })); for (const copy of copies) { const conflicts = await findTeamShiftConflicts(input.campaignId, copy.startsAt, copy.endsAt, memberIds); if (conflicts.length) throw new TRPCError({ code: "CONFLICT", message: `A cópia conflita com a agenda de ${conflicts.map(item => item.memberName).join(", ")}.` }); } const ids = await Promise.all(copies.map(copy => db.createTeamShift({ campaignId: input.campaignId, title: source.title, territory: source.territory, responsibility: source.responsibility, startsAt: copy.startsAt, endsAt: copy.endsAt, notes: source.notes, memberIds, createdByUserId: ctx.user.id }))); return { ids }; }),
    reschedule: protectedProcedure.input(campaignIdInput.extend({ shiftId: z.number().int().positive(), startsAt: z.date(), endsAt: z.date() })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); if (input.endsAt <= input.startsAt) throw new TRPCError({ code: "BAD_REQUEST", message: "O término deve ser posterior ao início." }); const shift = (await db.listTeamShifts(input.campaignId)).find(item => item.id === input.shiftId); if (!shift) throw new TRPCError({ code: "NOT_FOUND", message: "Escala não encontrada nesta campanha." }); const conflicts = await findTeamShiftConflicts(input.campaignId, input.startsAt, input.endsAt, shift.assignments.map(item => item.memberId), input.shiftId); if (conflicts.length) throw new TRPCError({ code: "CONFLICT", message: `Conflito de horário com ${conflicts.map(item => item.memberName).join(", ")}.` }); await db.rescheduleTeamShift(input.campaignId, input.shiftId, input.startsAt, input.endsAt); return { success: true }; }),
  ...
  executive: protectedProcedure.input(campaignIdInput).query(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); return db.getCampaignExecutiveSummary(input.campaignId); }),
});
