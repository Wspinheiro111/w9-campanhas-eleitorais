...
export const operationsRouter = router({
  availability: router({
    list: protectedProcedure.input(campaignIdInput).query(async ({ ctx, input }) => { await requireAccess(ctx.user.id, input.campaignId); return db.listMemberAvailabilities(input.campaignId); }),
    create: protectedProcedure.input(campaignIdInput.extend({ memberId: z.number().int().positive(), startsAt: z.date(), endsAt: z.date(), status: z.enum(["available", "preferred", "unavailable"]), note: z.string().max(500).optional() })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); if (access.member?.role === "partner" && access.member.id !== input.memberId) throw new TRPCError({ code: "FORBIDDEN", message: "Você só pode registrar a própria disponibilidade." }); if (input.endsAt <= input.startsAt) throw new TRPCError({ code: "BAD_REQUEST", message: "O término deve ser posterior ao início." }); const member = await db.getCampaignMember(input.campaignId, input.memberId); if (!member) throw new TRPCError({ code: "NOT_FOUND", message: "Membro não encontrado nesta campanha." }); return { id: await db.createMemberAvailability({ ...input, note: input.note ?? null }) }; }),
    remove: protectedProcedure.input(campaignIdInput.extend({ availabilityId: z.number().int().positive() })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "team"); await db.removeMemberAvailability(input.campaignId, input.availabilityId); return { success: true }; }),
  }),
  shifts: router({
    list: protectedProcedure.input(campaignIdInput).query(async ({ ctx, input }) => { await requireAccess(ctx.user.id, input.campaignId); return db.listTeamShifts(input.campaignId); }),
    create: protectedProcedure.input(campaignIdInput.extend({ title: z.string().min(3).max(220), territory: z.string().max(180).optional(), responsibility: z.string().max(220).optional(), startsAt: z.date(), endsAt: z.date(), notes: z.string().max(3000).optional(), memberIds: z.array(z.number().int().positive()).max(100).default([]), conflictOverrideReason: z.string().trim().min(10).max(1000).optional() })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "team"); if (input.endsAt <= input.startsAt) throw new TRPCError({ code: "BAD_REQUEST", message: "O término deve ser posterior ao início." }); const members = await db.listMembers(input.campaignId); const allowed = new Set(members.map(item => item.id)); if (input.memberIds.some(id => !allowed.has(id))) throw new TRPCError({ code: "BAD_REQUEST", message: "A escala contém membro de outra campanha." }); const conflicts = await findTeamShiftConflicts(input.campaignId, input.startsAt, input.endsAt, input.memberIds); if (conflicts.length && !input.conflictOverrideReason) throw new TRPCError({ code: "CONFLICT", message: `Conflito de horário com ${conflicts.map(item => item.memberName).join(", ")}. Informe uma justificativa para prosseguir.` }); return { id: await db.createTeamShift({ ...input, territory: input.territory ?? null, responsibility: input.responsibility ?? null, notes: input.notes ?? null, conflictOverrideReason: conflicts.length ? input.conflictOverrideReason : null, createdByUserId: ctx.user.id }) }; }),
    copy: protectedProcedure.input(campaignIdInput.extend({ sourceShiftId: z.number().int().positive(), targetStartsAt: z.array(z.date()).min(1).max(24), conflictOverrideReason: z.string().trim().min(10).max(1000).optional() })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); const source = (await db.listTeamShifts(input.campaignId)).find(item => item.id === input.sourceShiftId); if (!source) throw new TRPCError({ code: "NOT_FOUND", message: "Escala de origem não encontrada." }); const duration = source.endsAt.getTime() - source.startsAt.getTime(); const memberIds = source.assignments.map(item => item.memberId); const copies = input.targetStartsAt.map(startsAt => ({ startsAt, endsAt: new Date(startsAt.getTime() + duration) })); let hasConflicts = false; for (const copy of copies) { const conflicts = await findTeamShiftConflicts(input.campaignId, copy.startsAt, copy.endsAt, memberIds); if (conflicts.length) hasConflicts = true; } if (hasConflicts && !input.conflictOverrideReason) throw new TRPCError({ code: "CONFLICT", message: "Uma ou mais cópias conflitam com a agenda da equipe. Informe uma justificativa para prosseguir." }); const ids = await Promise.all(copies.map(copy => db.createTeamShift({ campaignId: input.campaignId, title: source.title, territory: source.territory, responsibility: source.responsibility, startsAt: copy.startsAt, endsAt: copy.endsAt, notes: source.notes, memberIds, conflictOverrideReason: hasConflicts ? input.conflictOverrideReason : null, createdByUserId: ctx.user.id }))); return { ids }; }),
    reschedule: protectedProcedure.input(campaignIdInput.extend({ shiftId: z.number().int().positive(), startsAt: z.date(), endsAt: z.date(), conflictOverrideReason: z.string().trim().min(10).max(1000).optional() })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); if (input.endsAt <= input.startsAt) throw new TRPCError({ code: "BAD_REQUEST", message: "O término deve ser posterior ao início." }); const shift = (await db.listTeamShifts(input.campaignId)).find(item => item.id === input.shiftId); if (!shift) throw new TRPCError({ code: "NOT_FOUND", message: "Escala não encontrada nesta campanha." }); const conflicts = await findTeamShiftConflicts(input.campaignId, input.startsAt, input.endsAt, shift.assignments.map(item => item.memberId), input.shiftId); if (conflicts.length && !input.conflictOverrideReason) throw new TRPCError({ code: "CONFLICT", message: `Conflito de horário com ${conflicts.map(item => item.memberName).join(", ")}. Informe uma justificativa para prosseguir.` }); await db.rescheduleTeamShift({ campaignId: input.campaignId, shiftId: input.shiftId, startsAt: input.startsAt, endsAt: input.endsAt, actorUserId: ctx.user.id, conflictOverrideReason: conflicts.length ? input.conflictOverrideReason : null }); return { success: true }; }),
    updateAssignmentStatus: protectedProcedure.input(campaignIdInput.extend({ assignmentId: z.number().int().positive(), status: z.enum(["assigned", "confirmed", "declined"]) })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); if (access.member?.role === "partner") { const shifts = await db.listTeamShifts(input.campaignId); const own = shifts.flatMap(shift => shift.assignments).find(item => item.id === input.assignmentId && item.memberId === access.member?.id); if (!own) throw new TRPCError({ code: "FORBIDDEN", message: "Você só pode atualizar a própria escala." }); } await db.updateTeamShiftAssignmentStatus(input.campaignId, input.assignmentId, input.status); return { success: true }; }),
  }),
  street: router({
    list: protectedProcedure.input(campaignIdInput).query(async ({ ctx, input }) => { await requireAccess(ctx.user.id, input.campaignId); return db.listStreetActions(input.campaignId); }),
    create: protectedProcedure.input(campaignIdInput.extend({ eventId: z.number().int().positive().optional(), title: z.string().trim().min(3).max(220), startsAt: z.date(), endsAt: z.date().optional(), territory: z.string().trim().max(180).optional(), neighborhood: z.string().trim().max(120).optional(), region: z.string().trim().max(120).optional(), responsibleMemberId: z.number().int().positive().optional(), notes: z.string().trim().max(3000).optional() })).mutation(async ({ ctx, input }) => {
      const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage");
      if (input.endsAt && input.endsAt <= input.startsAt) throw new TRPCError({ code: "BAD_REQUEST", message: "O término deve ser posterior ao início." });
      try { return { id: await db.createStreetAction({ ...input, eventId: input.eventId ?? null, endsAt: input.endsAt ?? null, territory: input.territory ?? null, neighborhood: input.neighborhood ?? null, region: input.region ?? null, responsibleMemberId: input.responsibleMemberId ?? null, notes: input.notes ?? null, createdByUserId: ctx.user.id }) }; }
      catch (error) { const code = error instanceof Error ? error.message : "STREET_ACTION_FAILED"; const messages: Record<string, string> = { STREET_ACTION_EVENT_INVALID: "O evento informado não pertence a esta campanha.", STREET_ACTION_MEMBER_INVALID: "O responsável informado não pertence a esta campanha." }; throw new TRPCError({ code: "BAD_REQUEST", message: messages[code] ?? "Não foi possível criar a ação de rua." }); }
    }),
    checkins: protectedProcedure.input(campaignIdInput.extend({ streetActionId: z.number().int().positive() })).query(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); return db.listStreetActionCheckins(input.campaignId, input.streetActionId); }),
    checkIn: protectedProcedure.input(campaignIdInput.extend({ streetActionId: z.number().int().positive(), memberId: z.number().int().positive(), attendanceStatus: z.enum(["present", "absent"]), notes: z.string().trim().max(1500).optional() })).mutation(async ({ ctx, input }) => {
      const access = await requireAccess(ctx.user.id, input.campaignId);
      if (access.member?.role === "partner" && access.member.id !== input.memberId) throw new TRPCError({ code: "FORBIDDEN", message: "Você só pode registrar a própria presença." });
      try { await db.saveStreetActionCheckin({ ...input, notes: input.notes ?? null, actorUserId: ctx.user.id }); return { success: true }; }
      catch (error) { const code = error instanceof Error ? error.message : "STREET_ACTION_CHECKIN_FAILED"; const messages: Record<string, string> = { STREET_ACTION_NOT_FOUND: "Ação de rua não encontrada nesta campanha.", STREET_ACTION_MEMBER_INVALID: "Membro não encontrado nesta campanha." }; throw new TRPCError({ code: "BAD_REQUEST", message: messages[code] ?? "Não foi possível salvar o check-in." }); }
    }),
  }),
  demands: router({
    list: protectedProcedure.input(campaignIdInput).query(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); if (access.member?.role === "partner") return db.listCommunityDemands(input.campaignId, access.member.id); requireCapability(access, "manage"); return db.listCommunityDemands(input.campaignId); }),
    create: protectedProcedure.input(campaignIdInput.extend({ sourceVoterId: z.number().int().positive().optional(), title: z.string().trim().min(3).max(220), description: z.string().trim().min(5).max(6000), category: z.string().trim().min(2).max(120), neighborhood: z.string().trim().max(120).optional(), region: z.string().trim().max(120).optional(), assignedToMemberId: z.number().int().positive().optional(), dueAt: z.date().optional() })).mutation(async ({ ctx, input }) => {
      const access = await requireAccess(ctx.user.id, input.campaignId);
      if (access.member?.role === "partner" && input.assignedToMemberId && access.member.id !== input.assignedToMemberId) throw new TRPCError({ code: "FORBIDDEN", message: "Você não pode atribuir a demanda a outro integrante." });
      try { return await db.createCommunityDemand({ ...input, sourceVoterId: input.sourceVoterId ?? null, neighborhood: input.neighborhood ?? null, region: input.region ?? null, assignedToMemberId: input.assignedToMemberId ?? (access.member?.role === "partner" ? access.member.id : null), dueAt: input.dueAt ?? null, createdByUserId: ctx.user.id, authorMemberId: access.member?.id ?? null }); }
      catch (error) { const code = error instanceof Error ? error.message : "COMMUNITY_DEMAND_FAILED"; const messages: Record<string, string> = { COMMUNITY_DEMAND_VOTER_INVALID: "O contato informado não pertence a esta campanha.", COMMUNITY_DEMAND_MEMBER_INVALID: "O responsável informado não pertence a esta campanha." }; throw new TRPCError({ code: "BAD_REQUEST", message: messages[code] ?? "Não foi possível registrar a demanda." }); }
    }),
    history: protectedProcedure.input(campaignIdInput.extend({ demandId: z.number().int().positive() })).query(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); return db.listCommunityDemandUpdates(input.campaignId, input.demandId); }),
    update: protectedProcedure.input(campaignIdInput.extend({ demandId: z.number().int().positive(), status: z.enum(["received", "triaged", "in_progress", "waiting_response", "returned", "closed"]), assignedToMemberId: z.number().int().positive().optional(), dueAt: z.date().optional(), note: z.string().trim().min(3).max(3000) })).mutation(async ({ ctx, input }) => {
      const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage");
      try { await db.updateCommunityDemand({ ...input, assignedToMemberId: input.assignedToMemberId ?? null, dueAt: input.dueAt ?? null, actorUserId: ctx.user.id, authorMemberId: access.member?.id ?? null }); return { success: true }; }
      catch (error) { const code = error instanceof Error ? error.message : "COMMUNITY_DEMAND_UPDATE_FAILED"; const messages: Record<string, string> = { COMMUNITY_DEMAND_NOT_FOUND: "Demanda não encontrada nesta campanha.", COMMUNITY_DEMAND_MEMBER_INVALID: "O responsável informado não pertence a esta campanha." }; throw new TRPCError({ code: "BAD_REQUEST", message: messages[code] ?? "Não foi possível atualizar a demanda." }); }
    }),
  }),
  materials: router({
    list: protectedProcedure.input(campaignIdInput).query(async ({ ctx, input }) => { await requireAccess(ctx.user.id, input.campaignId); return db.listCampaignMaterials(input.campaignId); }),
    movements: protectedProcedure.input(campaignIdInput.extend({ materialId: z.number().int().positive().optional() })).query(async ({ ctx, input }) => { await requireAccess(ctx.user.id, input.campaignId); return db.listCampaignMaterialMovements(input.campaignId, input.materialId); }),
    create: protectedProcedure.input(campaignIdInput.extend({ name: z.string().trim().min(2).max(180), category: z.string().trim().min(2).max(100), unit: z.string().trim().min(2).max(40).optional(), stockQuantity: z.number().int().min(0).max(1000000).default(0), minimumQuantity: z.number().int().min(0).max(1000000).default(0), notes: z.string().trim().max(2000).optional() })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); return { id: await db.createCampaignMaterial({ ...input, unit: input.unit ?? null, notes: input.notes ?? null, createdByUserId: ctx.user.id }) }; }),
    move: protectedProcedure.input(campaignIdInput.extend({ materialId: z.number().int().positive(), movementType: z.enum(["stock_in", "distribution", "return", "adjustment_in", "adjustment_out"]), quantity: z.number().int().min(1).max(1000000), eventId: z.number().int().positive().optional(), streetActionId: z.number().int().positive().optional(), recipientMemberId: z.number().int().positive().optional(), territory: z.string().trim().max(180).optional(), notes: z.string().trim().max(2000).optional(), occurredAt: z.date().optional() })).mutation(async ({ ctx, input }) => {
      const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage");
      try { return { id: await db.recordCampaignMaterialMovement({ ...input, eventId: input.eventId ?? null, streetActionId: input.streetActionId ?? null, recipientMemberId: input.recipientMemberId ?? null, territory: input.territory ?? null, notes: input.notes ?? null, createdByUserId: ctx.user.id }) }; }
      catch (error) { const code = error instanceof Error ? error.message : "MATERIAL_MOVEMENT_FAILED"; const messages: Record<string, string> = { MATERIAL_NOT_FOUND: "Material não encontrado nesta campanha.", MATERIAL_EVENT_INVALID: "O evento informado não pertence a esta campanha.", MATERIAL_STREET_ACTION_INVALID: "A ação de rua informada não pertence a esta campanha.", MATERIAL_MEMBER_INVALID: "O destinatário informado não pertence a esta campanha.", MATERIAL_INSUFFICIENT_STOCK: "Estoque insuficiente para esta saída." }; throw new TRPCError({ code: "BAD_REQUEST", message: messages[code] ?? "Não foi possível registrar a movimentação." }); }
    }),
  }),
  notifications: router({
    list: protectedProcedure.input(campaignIdInput).query(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); return db.listCampaignNotifications(input.campaignId, access.member?.role === "partner" ? access.member.id : null); }),
    markRead: protectedProcedure.input(campaignIdInput).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); await db.markCampaignNotificationsRead(input.campaignId, access.member?.role === "partner" ? access.member.id : null); return { success: true }; }),
  }),
  reportTemplates: router({
    list: protectedProcedure.input(campaignIdInput).query(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); return db.listCampaignReportTemplates(input.campaignId); }),
    create: protectedProcedure.input(campaignIdInput.extend({ name: z.string().min(3).max(160), reportType: z.string().min(2).max(80).default("operational"), subtitle: z.string().max(220).optional(), coverNotes: z.string().max(4000).optional(), sections: z.array(z.string().min(1).max(120)).max(20).default([]) })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); return { id: await db.createCampaignReportTemplate({ ...input, subtitle: input.subtitle ?? null, coverNotes: input.coverNotes ?? null, createdByUserId: ctx.user.id }) }; }),
    remove: protectedProcedure.input(campaignIdInput.extend({ templateId: z.number().int().positive() })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); await db.removeCampaignReportTemplate(input.campaignId, input.templateId); return { success: true }; }),
  }),
  executive: protectedProcedure.input(campaignIdInput).query(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); const [summary, plannedVsRealized] = await Promise.all([db.getCampaignExecutiveSummary(input.campaignId), db.getPlannedVsRealizedCoverage(input.campaignId)]); return { ...summary, plannedVsRealized }; }),
});
