...
export const operationsRouter = router({
  ...
  shifts: router({
    ...
    create: protectedProcedure.input(campaignIdInput.extend({ title: z.string().min(3).max(220), territory: z.string().max(180).optional(), responsibility: z.string().max(220).optional(), startsAt: z.date(), endsAt: z.date(), notes: z.string().max(3000).optional(), memberIds: z.array(z.number().int().positive()).max(100).default([]) })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "team"); if (input.endsAt <= input.startsAt) throw new TRPCError({ code: "BAD_REQUEST", message: "O término deve ser posterior ao início." }); const members = await db.listMembers(input.campaignId); const allowed = new Set(members.map(item => item.id)); if (input.memberIds.some(id => !allowed.has(id))) throw new TRPCError({ code: "BAD_REQUEST", message: "A escala contém membro de outra campanha." }); return { id: await db.createTeamShift({ ...input, territory: input.territory ?? null, responsibility: input.responsibility ?? null, notes: input.notes ?? null, createdByUserId: ctx.user.id }) }; }),
    reschedule: protectedProcedure.input(campaignIdInput.extend({ shiftId: z.number().int().positive(), startsAt: z.date(), endsAt: z.date() })).mutation(async ({ ctx, input }) => { const access = await requireAccess(ctx.user.id, input.campaignId); requireCapability(access, "manage"); if (input.endsAt <= input.startsAt) throw new TRPCError({ code: "BAD_REQUEST", message: "O término deve ser posterior ao início." }); const shift = (await db.listTeamShifts(input.campaignId)).find(item => item.id === input.shiftId); if (!shift) throw new TRPCError({ code: "NOT_FOUND", message: "Escala não encontrada nesta campanha." }); await db.rescheduleTeamShift(input.campaignId, input.shiftId, input.startsAt, input.endsAt); return { success: true }; }),
