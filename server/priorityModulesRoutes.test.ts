...
describe("módulos prioritários", () => {
  ...
  it("retorna somente o portal do voluntário associado ao token privado", async () => {
    vi.mocked(db.getVolunteerByAccessTokenHash).mockResolvedValue({ id: 81, campaignId: 1, name: "Ana Voluntária", neighborhood: "Centro", region: "Norte", availability: "Sábado", skills: "Eventos", trainingStatus: "in_progress", status: "active" } as never); vi.mocked(db.listVolunteerAssignments).mockResolvedValue([{ id: 90, volunteerId: 81, title: "Apoiar reunião", status: "assigned" }] as never); vi.mocked(db.listVolunteerTrainingMaterials).mockResolvedValue([{ id: 91, title: "Conduta", materialType: "guide", durationMinutes: 10, completedAt: null }] as never); vi.mocked(db.getVolunteerTrainingCertificate).mockResolvedValue(null); vi.mocked(db.listVolunteerTrainingCertificates).mockResolvedValue([]); vi.mocked(db.getCampaignCertificateSettings).mockResolvedValue({ primaryColor: "#103527", accentColor: "#c9a85b", logoUrl: null, signatureName: null, signatureRole: null } as never);
    await expect(appRouter.createCaller(context()).volunteers.portal({ token: "x".repeat(32) })).resolves.toEqual(expect.objectContaining({ volunteer: expect.objectContaining({ name: "Ana Voluntária", trainingStatus: "in_progress" }), assignments: [expect.objectContaining({ id: 90, title: "Apoiar reunião" })], certificate: null, certificateHistory: [], certificateSettings: expect.objectContaining({ primaryColor: "#103527" }) }));
  });

  it("entrega todas as versões históricas do certificado somente no portal do token do voluntário", async () => {
    vi.mocked(db.getVolunteerByAccessTokenHash).mockResolvedValue({ id: 81, campaignId: 1, name: "Ana Voluntária", trainingStatus: "completed", status: "active" } as never); vi.mocked(db.listVolunteerAssignments).mockResolvedValue([]); vi.mocked(db.listVolunteerTrainingMaterials).mockResolvedValue([]); vi.mocked(db.getVolunteerTrainingCertificate).mockResolvedValue({ certificateCode: "W9-V2", issuedAt: new Date("2026-08-20T12:00:00Z"), completedMaterials: 3 } as never); vi.mocked(db.listVolunteerTrainingCertificates).mockResolvedValue([{ certificateCode: "W9-V2", issuedAt: new Date("2026-08-20T12:00:00Z"), completedMaterials: 3, versionNumber: 2 }, { certificateCode: "W9-V1", issuedAt: new Date("2026-08-19T12:00:00Z"), completedMaterials: 2, versionNumber: 1 }] as never); vi.mocked(db.getCampaignCertificateSettings).mockResolvedValue({ primaryColor: "#103527", accentColor: "#c9a85b", logoUrl: null, signatureName: null, signatureRole: null } as never);
    const result = await appRouter.createCaller(context()).volunteers.portal({ token: "y".repeat(32) });
    expect(result.certificateHistory).toHaveLength(2); expect(result.certificateHistory.map(item => item.certificateCode)).toEqual(["W9-V2", "W9-V1"]);
  });
  ...
  it("reserva prazos e indicadores de treinamento à coordenação com filtros de equipe e região", async () => {
    const dueAt = new Date("2026-09-15T23:59:00Z"); vi.mocked(db.getVolunteerTrainingMaterial).mockResolvedValue({ id: 91, campaignId: 1 } as never); vi.mocked(db.getCampaignAccess).mockResolvedValue({ campaign, member: partnerMember, organizationMember: { role: "operator" } } as never);
    await expect(appRouter.createCaller(context(12)).volunteers.training.updateDeadline({ materialId: 91, dueAt })).rejects.toMatchObject({ code: "FORBIDDEN" });
    vi.mocked(db.getCampaignAccess).mockResolvedValue({ campaign, member: adminMember, organizationMember: { role: "admin" } } as never); vi.mocked(db.updateVolunteerTrainingMaterialDeadline).mockResolvedValue(undefined); vi.mocked(db.getVolunteerTrainingDashboard).mockResolvedValue({ summary: { volunteers: 2, totalMaterials: 3, completedVolunteers: 1, completionRate: 50, overdueVolunteers: 1 }, byRegion: [{ region: "Norte", volunteers: 2, completionRate: 50, overdue: 1 }], volunteers: [] } as never);
    await expect(appRouter.createCaller(context()).volunteers.training.updateDeadline({ materialId: 91, dueAt })).resolves.toEqual({ success: true });
    await expect(appRouter.createCaller(context()).volunteers.training.dashboard({ campaignId: 1, coordinatorMemberId: 10, region: "Norte" })).resolves.toEqual(expect.objectContaining({ summary: expect.objectContaining({ completionRate: 50 }) }));
    expect(db.updateVolunteerTrainingMaterialDeadline).toHaveBeenCalledWith(91, dueAt); expect(db.getVolunteerTrainingDashboard).toHaveBeenCalledWith(1, { coordinatorMemberId: 10, region: "Norte" });
  });

  it("permite editar e reordenar a trilha somente no escopo da campanha", async () => {
    vi.mocked(db.getVolunteerTrainingMaterial).mockResolvedValue({ id: 91, campaignId: 1, title: "Conduta", active: true } as never); vi.mocked(db.getCampaignAccess).mockResolvedValue({ campaign, member: partnerMember, organizationMember: { role: "operator" } } as never);
    await expect(appRouter.createCaller(context(12)).volunteers.training.update({ materialId: 91, title: "Conduta revisada" })).rejects.toMatchObject({ code: "FORBIDDEN" });
    vi.mocked(db.getCampaignAccess).mockResolvedValue({ campaign, member: adminMember, organizationMember: { role: "admin" } } as never); vi.mocked(db.updateVolunteerTrainingMaterial).mockResolvedValue(undefined); vi.mocked(db.listVolunteerTrainingMaterials).mockResolvedValue([{ id: 91 }, { id: 92 }] as never); vi.mocked(db.reorderVolunteerTrainingMaterials).mockResolvedValue(undefined);
    await expect(appRouter.createCaller(context()).volunteers.training.update({ materialId: 91, title: "Conduta revisada", active: false })).resolves.toEqual({ success: true });
    await expect(appRouter.createCaller(context()).volunteers.training.reorder({ campaignId: 1, materialIds: [92, 91] })).resolves.toEqual({ success: true });
    expect(db.updateVolunteerTrainingMaterial).toHaveBeenCalledWith(91, { title: "Conduta revisada", active: false }); expect(db.reorderVolunteerTrainingMaterials).toHaveBeenCalledWith(1, [92, 91]);
  });

  it("protege metas e ranking mensal de formação por equipe", async () => {
    vi.mocked(db.getCampaignAccess).mockResolvedValue({ campaign, member: partnerMember, organizationMember: { role: "operator" } } as never);
    await expect(appRouter.createCaller(context(12)).volunteers.training.monthlyRanking({ campaignId: 1, month: "2026-08" })).rejects.toMatchObject({ code: "FORBIDDEN" });
    vi.mocked(db.getCampaignAccess).mockResolvedValue({ campaign, member: adminMember, organizationMember: { role: "admin" } } as never); vi.mocked(db.getCampaignMember).mockResolvedValue({ id: 10, campaignId: 1 } as never); vi.mocked(db.setVolunteerTrainingTeamGoal).mockResolvedValue(120); vi.mocked(db.getVolunteerTrainingTeamRanking).mockResolvedValue([{ coordinatorMemberId: 10, name: "Equipe Norte", assignedVolunteers: 3, completedTrainingsThisMonth: 2, targetCompletions: 8, goalProgress: 25, hasGoal: true }] as never);
    await expect(appRouter.createCaller(context()).volunteers.training.setMonthlyGoal({ campaignId: 1, coordinatorMemberId: 10, month: "2026-08", targetCompletions: 8 })).resolves.toEqual({ id: 120 });
    await expect(appRouter.createCaller(context()).volunteers.training.monthlyRanking({ campaignId: 1, month: "2026-08" })).resolves.toEqual([expect.objectContaining({ name: "Equipe Norte", completedTrainingsThisMonth: 2, goalProgress: 25 })]);
    expect(db.setVolunteerTrainingTeamGoal).toHaveBeenCalledWith({ campaignId: 1, coordinatorMemberId: 10, month: "2026-08", targetCompletions: 8 });
  });

  it("reserva regras e histórico de medalhas à coordenação da campanha", async () => {
    vi.mocked(db.getCampaignAccess).mockResolvedValue({ campaign, member: partnerMember, organizationMember: { role: "operator" } } as never);
    await expect(appRouter.createCaller(context(12)).volunteers.training.recognition.rules({ campaignId: 1 })).rejects.toMatchObject({ code: "FORBIDDEN" });
    vi.mocked(db.getCampaignAccess).mockResolvedValue({ campaign, member: adminMember, organizationMember: { role: "admin" } } as never); vi.mocked(db.updateCampaignTrainingRecognitionRules).mockResolvedValue(41); vi.mocked(db.recordVolunteerTrainingRecognitionHistory).mockResolvedValue({ recorded: 2 }); vi.mocked(db.listVolunteerTrainingRecognitionHistory).mockResolvedValue([{ id: 1, month: "2026-08", name: "Equipe Norte", medal: "leader" }] as never);
    await expect(appRouter.createCaller(context()).volunteers.training.recognition.updateRules({ campaignId: 1, achievedThreshold: 100, standoutThreshold: 130 })).resolves.toEqual({ id: 41 });
    await expect(appRouter.createCaller(context()).volunteers.training.recognition.recordHistory({ campaignId: 1, month: "2026-08" })).resolves.toEqual({ recorded: 2 });
    await expect(appRouter.createCaller(context()).volunteers.training.recognition.history({ campaignId: 1 })).resolves.toEqual([expect.objectContaining({ medal: "leader" })]);
  });
  ...
  it("reserva a identidade visual do certificado à coordenação da campanha", async () => {
    vi.mocked(db.getCampaignAccess).mockResolvedValue({ campaign, member: partnerMember, organizationMember: { role: "operator" } } as never);
    await expect(appRouter.createCaller(context(12)).volunteers.certificates.settings.update({ campaignId: 1, primaryColor: "#103527", accentColor: "#c9a85b" })).rejects.toMatchObject({ code: "FORBIDDEN" });
