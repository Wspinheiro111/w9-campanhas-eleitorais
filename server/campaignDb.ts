...
export async function getFinancialSummary(campaignId: number) { const entries = await listFinancialEntries(campaignId); const active = entries.filter(entry => !["rejected", "cancelled"].includes(entry.status)); const incomeCents = active.filter(entry => entry.entryType === "income").reduce((sum, entry) => sum + entry.amountCents, 0); const expenseCents = active.filter(entry => entry.entryType === "expense").reduce((sum, entry) => sum + entry.amountCents, 0); return { incomeCents, expenseCents, balanceCents: incomeCents - expenseCents, paidIncomeCents: active.filter(entry => entry.entryType === "income" && entry.status === "paid").reduce((sum, entry) => sum + entry.amountCents, 0), paidExpenseCents: active.filter(entry => entry.entryType === "expense" && entry.status === "paid").reduce((sum, entry) => sum + entry.amountCents, 0), pendingCount: entries.filter(entry => ["pending", "under_review"].includes(entry.status)).length }; }
export async function createFinancialEntry(input: { campaignId: number; createdByUserId: number; entryType: "income" | "expense"; category: string; counterpartyName: string; counterpartyDocument?: string | null; amountCents: number; paymentMethod?: string | null; receiptNumber?: string | null; documentNumber?: string | null; dueDate?: Date | null; paidAt?: Date | null; notes?: string | null }) { const db = requireDb(await getDb()); const organizationId = await organizationIdForCampaign(input.campaignId); const result = await db.insert(campaignFinancialEntries).values({ ...input, organizationId, counterpartyDocument: input.counterpartyDocument ?? null, paymentMethod: input.paymentMethod ?? null, receiptNumber: input.receiptNumber ?? null, documentNumber: input.documentNumber ?? null, dueDate: input.dueDate ?? null, paidAt: input.paidAt ?? null, notes: input.notes ?? null, status: input.paidAt ? "paid" : "pending" }); return Number(result[0].insertId); }
...
export async function updateFinancialEntryReview(input: { id: number; status: "under_review" | "approved" | "rejected" | "paid" | "cancelled"; reviewedByUserId: number; reviewNotes?: string | null }) { const db = requireDb(await getDb()); await db.update(campaignFinancialEntries).set({ status: input.status, reviewedByUserId: input.reviewedByUserId, reviewedAt: new Date(), reviewNotes: input.reviewNotes ?? null, paidAt: input.status === "paid" ? new Date() : undefined }).where(eq(campaignFinancialEntries.id, input.id)); }
