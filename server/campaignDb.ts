...
export async function getPlannedVsRealizedCoverage(campaignId: number) { const shifts = await listTeamShifts(campaignId); return Array.from(shifts.filter(shift => shift.status !== "cancelled").reduce((groups, shift) => { const territory = shift.territory || "Território a definir"; const current = groups.get(territory) ?? { territory, plannedShifts: 0, plannedAssignments: 0, realizedShifts: 0, realizedAssignments: 0 }; current.plannedShifts += 1; current.plannedAssignments += shift.assignments.length; if (shift.status === "completed") { current.realizedShifts += 1; current.realizedAssignments += shift.assignments.filter(assignment => assignment.status === "confirmed").length; } groups.set(territory, current); return groups; }, new Map<string, { territory: string; plannedShifts: number; plannedAssignments: number; realizedShifts: number; realizedAssignments: number }>()).values()).map(item => ({ ...item, realizationRate: item.plannedAssignments ? Math.round((item.realizedAssignments / item.plannedAssignments) * 100) : 0 })).sort((a, b) => b.plannedAssignments - a.plannedAssignments || a.territory.localeCompare(b.territory)); }

export async function createTeamShift(input: { campaignId: number; title: string; territory: string | null; responsibility: string | null; startsAt: Date; endsAt: Date; notes: string | null; memberIds: number[]; createdByUserId: number; conflictOverrideReason?: string | null }) {
  const db = requireDb(await getDb()); const organizationId = await organizationIdForCampaign(input.campaignId);
  const result = await db.insert(campaignTeamShifts).values({ organizationId, campaignId: input.campaignId, title: input.title, territory: input.territory, responsibility: input.responsibility, startsAt: input.startsAt, endsAt: input.endsAt, notes: input.notes, conflictOverrideReason: input.conflictOverrideReason ?? null, conflictOverriddenByUserId: input.conflictOverrideReason ? input.createdByUserId : null, conflictOverriddenAt: input.conflictOverrideReason ? new Date() : null, createdByUserId: input.createdByUserId }); const shiftId = Number(result[0].insertId);
  ...
  if (input.conflictOverrideReason) await createOrganizationAuditLog({ organizationId, actorUserId: input.createdByUserId, action: "team_shift.conflict_overridden", entityType: "team_shift", entityId: shiftId, metadata: { reason: input.conflictOverrideReason } });
...
export async function rescheduleTeamShift(input: { campaignId: number; shiftId: number; startsAt: Date; endsAt: Date; actorUserId: number; conflictOverrideReason?: string | null }) { const db = requireDb(await getDb()); await db.update(campaignTeamShifts).set({ startsAt: input.startsAt, endsAt: input.endsAt, conflictOverrideReason: input.conflictOverrideReason ?? null, conflictOverriddenByUserId: input.conflictOverrideReason ? input.actorUserId : null, conflictOverriddenAt: input.conflictOverrideReason ? new Date() : null, updatedAt: new Date() }).where(and(eq(campaignTeamShifts.id, input.shiftId), eq(campaignTeamShifts.campaignId, input.campaignId))); if (input.conflictOverrideReason) await createOrganizationAuditLog({ organizationId: await organizationIdForCampaign(input.campaignId), actorUserId: input.actorUserId, action: "team_shift.conflict_overridden", entityType: "team_shift", entityId: input.shiftId, metadata: { reason: input.conflictOverrideReason, operation: "reschedule" } }); }
...
export async function getCampaignExecutiveSummary(campaignId: number) { const [dashboard, shifts, notifications, templates] = await Promise.all([getDashboardData(campaignId), listTeamShifts(campaignId), listCampaignNotifications(campaignId), listCampaignReportTemplates(campaignId)]); const now = new Date(); const upcomingShifts = shifts.filter(item => item.startsAt > now && item.status === "scheduled"); const unconfirmedAssignments = upcomingShifts.flatMap(item => item.assignments).filter(item => item.status === "assigned").length; const territoryCoverage = Array.from(upcomingShifts.reduce((groups, shift) => { const territory = shift.territory || "Território a definir"; const current = groups.get(territory) ?? { territory, shifts: 0, assigned: 0, confirmed: 0 }; current.shifts += 1; current.assigned += shift.assignments.length; current.confirmed += shift.assignments.filter(item => item.status === "confirmed").length; groups.set(territory, current); return groups; }, new Map<string, { territory: string; shifts: number; assigned: number; confirmed: number }>()).values()).map(item => ({ ...item, coverageRate: item.assigned ? Math.round((item.confirmed / item.assigned) * 100) : 0 })).sort((a, b) => b.shifts - a.shifts || b.confirmed - a.confirmed); const weeklyTerritoryTrend = Array.from(upcomingShifts.reduce((groups, shift) => { const weekStart = new Date(shift.startsAt); weekStart.setHours(0, 0, 0, 0); const weekday = weekStart.getDay(); weekStart.setDate(weekStart.getDate() - (weekday === 0 ? 6 : weekday - 1)); const week = weekStart.toISOString().slice(0, 10); const territory = shift.territory || "Território a definir"; const key = `${week}::${territory}`; const current = groups.get(key) ?? { week, territory, shifts: 0, assigned: 0, confirmed: 0 }; current.shifts += 1; current.assigned += shift.assignments.length; current.confirmed += shift.assignments.filter(item => item.status === "confirmed").length; groups.set(key, current); return groups; }, new Map<string, { week: string; territory: string; shifts: number; assigned: number; confirmed: number }>()).values()).map(item => ({ ...item, coverageRate: item.assigned ? Math.round((item.confirmed / item.assigned) * 100) : 0 })).sort((a, b) => a.week.localeCompare(b.week) || a.territory.localeCompare(b.territory)); return { dashboard, execution: { upcomingShifts: upcomingShifts.length, unconfirmedAssignments, unreadNotifications: notifications.filter(item => !item.readAt).length, reportTemplates: templates.length }, territoryCoverage, weeklyTerritoryTrend, priorities: notifications.filter(item => !item.readAt).slice(0, 5), upcomingShifts: upcomingShifts.slice(0, 5) }; }
