...
import { Activity, Building2, CheckCircle2, ShieldCheck } from "lucide-react";
...
const labels: Record<string, string> = {
  ...
  "finance.entry.review_conflict": "Conflito de atualização financeira",
};
...
export default function AuditLog() {
  ...
  const securityReports = trpc.organization.audit.securityReport.useQuery({ organizationId: activeOrganizationId || 0 }, { enabled: Boolean(activeOrganizationId && canView) });
  ...
  return <><header className="mb-7"><p className="text-xs font-bold uppercase tracking-[.16em] text-primary">Governança da organização</p><h1 className="mt-2 font-serif text-3xl tracking-tight">Histórico administrativo</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Acompanhe as principais ações administrativas da organização <strong className="font-medium text-foreground">{active.organization.name}</strong>.</p></header>
    ...
    <section className="mt-6 rounded-2xl border border-border bg-card"><div className="flex items-center gap-3 border-b border-border p-5"><span className="flex size-10 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-700"><ShieldCheck className="size-5" /></span><div><h2 className="font-semibold">Relatórios de segurança por versão</h2><p className="mt-1 text-xs text-muted-foreground">Evidências técnicas registradas a cada versão auditada do sistema.</p></div></div>{securityReports.data?.map(report => <article key={report.version} className="p-5"><div className="flex flex-wrap items-center gap-2"><h3 className="font-semibold">{report.title}</h3><Badge variant="outline">Versão {report.version}</Badge><time className="text-xs text-muted-foreground">{new Date(report.publishedAt).toLocaleString("pt-BR")}</time></div><div className="mt-4 grid gap-2 md:grid-cols-2">{report.checks.map(check => <div key={check.id} className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3 text-sm"><p className="flex items-center gap-2 font-medium"><CheckCircle2 className="size-4 text-emerald-700" />{check.label}</p><p className="mt-1 text-muted-foreground">{check.detail}</p></div>)}</div></article>)}{securityReports.isLoading && <div className="h-32 animate-pulse bg-muted/50" />}</section>
  </>;
}
