...
function TerritoryContent() {
  ...
  const queryInput = useMemo(() => ({ campaignId: activeCampaign!.id, startsAt: startsAt ? new Date(`${startsAt}T00:00:00`) : undefined, endsAt: endsAt ? new Date(`${endsAt}T23:59:59`) : undefined, memberId: memberId ? Number(memberId) : undefined }), [activeCampaign, startsAt, endsAt, memberId]);
  const { data, isLoading } = trpc.territory.overview.useQuery(queryInput);
  ...
  const clearFilters = () => { setStartsAt(""); setEndsAt(""); setMemberId(""); };
  return <><PageHeader eyebrow="Inteligência territorial" title="Mapa de território" description="Visualize a presença da campanha por bairro e região, com sinais de agenda e ocorrências de campo." />
    <section className="mb-6 grid gap-3 rounded-2xl border border-border bg-card p-4 md:grid-cols-4"><div className="space-y-1.5"><Label className="text-xs">Início</Label><Input type="date" value={startsAt} onChange={event => setStartsAt(event.target.value)} /></div><div className="space-y-1.5"><Label className="text-xs">Fim</Label><Input type="date" value={endsAt} onChange={event => setEndsAt(event.target.value)} /></div><div className="space-y-1.5"><Label className="text-xs">Responsável</Label><select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={memberId} onChange={event => setMemberId(event.target.value)}><option value="">Toda a equipe</option>{members?.map(member => <option key={member.id} value={member.id}>{member.name}</option>)}</select></div><div className="flex items-end"><Button variant="outline" className="w-full" onClick={clearFilters}>Limpar filtros</Button></div></section>
    ...
    <section className="mt-6 overflow-hidden rounded-2xl border border-border bg-card"><div className="flex items-center gap-3 border-b border-border p-5"><span className="flex size-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><MapPinned className="size-5" /></span><div><h2 className="font-semibold">Distribuição geográfica</h2><p className="mt-1 text-xs text-muted-foreground">Os marcadores representam registros agregados, sem expor endereços individuais.</p></div></div>{territories.length ? <MapView key={`${startsAt}-${endsAt}-${memberId}`} className="h-[460px]" initialCenter={{ lat: -14.235, lng: -51.9253 }} initialZoom={4} onMapReady={onMapReady} /> : <div className="flex h-[360px] flex-col items-center justify-center bg-[radial-gradient(circle_at_50%_40%,rgba(201,168,91,.16),transparent_28%),linear-gradient(135deg,#f8f7f2,#f1f4ef)] p-8 text-center"><span className="flex size-14 items-center justify-center rounded-2xl bg-primary/10 text-primary"><MapPinned className="size-7" /></span><h3 className="mt-5 font-serif text-2xl">Nenhum registro para estes filtros</h3><p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">Ajuste o período ou o responsável, ou registre bairro e região nos contatos para alimentar a leitura geográfica.</p></div>}</section>
