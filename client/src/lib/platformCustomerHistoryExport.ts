...
export function buildCommercialHistoryPrintDocument(input: { organizationName: string; contactName: string; contactPhone: string; nextContactAt?: Date | string | null; nextContactNote?: string | null; interactions: CommercialInteractionExport[] }) {
  ...
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8" /><title>Histórico comercial - ${escapeHtml(input.organizationName)}</title><style>${w9PrintStyles}</style></head><body>${w9PrintHeader("Histórico comercial")}<p class="w9-meta">${escapeHtml(input.organizationName)} · ${escapeHtml(input.contactName)} · ${escapeHtml(input.contactPhone)}</p><section class="next"><strong>Próximo contato:</strong> ${escapeHtml(nextContact)}${input.nextContactNote ? `<br /><span>${escapeHtml(input.nextContactNote)}</span>` : ""}</section><h2>Interações</h2>${rows}</body></html>`;
}
